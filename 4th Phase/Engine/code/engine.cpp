...
int timebase = 0, frame = 0, counter_l = 0, window;
...
Group* readXML(const char* file) {
	...
	if (camara != NULL) {
		...
		raio = sqrtf(powf(d[0], 2) + powf(d[1], 2) + powf(d[2], 2));
		beta = asin(posY / raio);
...
void processSpecialKeys(int key, int xx, int yy) {
	...
	switch (key) {
		...
		case GLUT_KEY_PAGE_DOWN: raio -= 5.0f;
			if (raio < 1.0f) {
				raio = 1.0f;
			}
			glutPostRedisplay();
...
// init GLUT and the window
	...
	window = glutCreateWindow("Engine - Phase 4");
