...
void esfera(float r, int slices, int stacks, char* fname) {
    ...
    float px3, py3, pz3, nx3, ny3, nz3;
    float px4, py4, pz4, nx4, ny4, nz4;

    for (int slice = 0; slice < slices; slice++) {
        px1 = r * cos(beta + beta_step) * sin(alpha);
        py1 = r * sin(beta + beta_step);
        pz1 = r * cos(beta + beta_step) * cos(alpha);
        nx1 = cos(beta + beta_step) * sin(alpha);
        ny1 = sin(beta + beta_step);
        nz1 = cos(beta + beta_step) * cos(alpha);
        ...
        px3 = r * cos(beta + beta_step) * sin(alpha + alpha_step);
        py3 = r * sin(beta + beta_step);
        pz3 = r * cos(beta + beta_step) * cos(alpha + alpha_step);
        nx3 = cos(beta + beta_step) * sin(alpha + alpha_step);
        ny3 = sin(beta + beta_step);
        nz3 = cos(beta + beta_step) * cos(alpha + alpha_step);

        fprintf(f, "%f %f %f %f %f %f\n", px1, py1, pz1, nx1, ny1, nz1);
        fprintf(f, "%f %f %f %f %f %f\n", px2, py2, pz2, nx2, ny2, nz2);
        fprintf(f, "%f %f %f %f %f %f\n", px3, py3, pz3, nx3, ny3, nz3);
    ...
    for (int stack = 1; stack < stacks - 1; stack++) {
        alpha = 0.0f;
        for (int slice = 0; slice < slices; slice++) {
            px1 = r * cos(beta) * sin(alpha);
            py1 = r * sin(beta);
            pz1 = r * cos(beta) * cos(alpha);
            nx1 = cos(beta) * sin(alpha);
            ny1 = sin(beta);
            nz1 = cos(beta) * cos(alpha);

            px2 = r * cos(beta) * sin(alpha + alpha_step);
            py2 = r * sin(beta);
            pz2 = r * cos(beta) * cos(alpha + alpha_step);
            nx2 = cos(beta) * sin(alpha + alpha_step);
            ny2 = sin(beta);
            nz2 = cos(beta) * cos(alpha + alpha_step);
            ...
            px4 = r * cos(beta + beta_step) * sin(alpha);
            py4 = r * sin(beta + beta_step);
            pz4 = r * cos(beta + beta_step) * cos(alpha);
            nx4 = cos(beta + beta_step) * sin(alpha);
            ny4 = sin(beta + beta_step);
            nz4 = cos(beta + beta_step) * cos(alpha);


            fprintf(f, "%f %f %f %f %f %f\n", px1, py1, pz1, nx1, ny1, nz1);
            fprintf(f, "%f %f %f %f %f %f\n", px2, py2, pz2, nx2, ny2, nz2);
            fprintf(f, "%f %f %f %f %f %f\n", px4, py4, pz4, nx4, ny4, nz4);
    ...
    for (int slice = 0; slice < slices; slice++) {
        ...
        px2 = 0.0f;
        py2 = r * sin(beta + beta_step);
        pz2 = 0.0f;
        nx2 = 0.0f;
        ny2 = 1.0f;
        nz2 = 0.0f;

        px3 = r * cos(beta) * sin(alpha + alpha_step);
        py3 = r * sin(beta);
        pz3 = r * cos(beta) * cos(alpha + alpha_step);
        nx3 = cos(beta) * sin(alpha + alpha_step);
        ny3 = sin(beta);
        nz3 = cos(beta) * cos(alpha + alpha_step);
...
void getderiU(float u, float v, int i, float* deru) {
    ...
    float matrizX[4][4] = { { triangles[sup[i].p1].xx, triangles[sup[i].p2].xx, triangles[sup[i].p3].xx, triangles[sup[i].p4].xx},
                            { triangles[sup[i].p5].xx, triangles[sup[i].p6].xx, triangles[sup[i].p7].xx, triangles[sup[i].p8].xx },
                            { triangles[sup[i].p9].xx, triangles[sup[i].p10].xx, triangles[sup[i].p11].xx, triangles[sup[i].p12].xx},
                            { triangles[sup[i].p13].xx, triangles[sup[i].p14].xx, triangles[sup[i].p15].xx, triangles[sup[i].p16].xx} };


    float matrizY[4][4] = { { triangles[sup[i].p1].yy, triangles[sup[i].p2].yy, triangles[sup[i].p3].yy, triangles[sup[i].p4].yy},
                            { triangles[sup[i].p5].yy, triangles[sup[i].p6].yy, triangles[sup[i].p7].yy, triangles[sup[i].p8].yy },
                            { triangles[sup[i].p9].yy, triangles[sup[i].p10].yy, triangles[sup[i].p11].yy, triangles[sup[i].p12].yy},
                            { triangles[sup[i].p13].yy, triangles[sup[i].p14].yy, triangles[sup[i].p15].yy, triangles[sup[i].p16].yy} };

    float matrizZ[4][4] = { { triangles[sup[i].p1].zz, triangles[sup[i].p2].zz, triangles[sup[i].p3].zz, triangles[sup[i].p4].zz},
                            { triangles[sup[i].p5].zz, triangles[sup[i].p6].zz, triangles[sup[i].p7].zz, triangles[sup[i].p8].zz },
                            { triangles[sup[i].p9].zz, triangles[sup[i].p10].zz, triangles[sup[i].p11].zz, triangles[sup[i].p12].zz},
                            { triangles[sup[i].p13].zz, triangles[sup[i].p14].zz, triangles[sup[i].p15].zz, triangles[sup[i].p16].zz} };
